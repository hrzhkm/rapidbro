...
const MYBAS_KUALA_TERENGGANU_GTFS_RT_URL: &str =
    "https://api.data.gov.my/gtfs-realtime/vehicle-position/mybas-kuala-terengganu";

/// TTL for cached bus data: 5 minutes.
const KUALA_TERENGGANU_DATA_TTL_MS: i64 = 300_000;

pub const REDIS_MYBAS_KUALA_TERENGGANU_LATEST_KEY: &str =
    "rapidbro:mybas-kuala-terengganu:buses:latest";
pub const REDIS_MYBAS_KUALA_TERENGGANU_LAST_SEEN_KEY: &str =
    "rapidbro:mybas-kuala-terengganu:buses:last_seen";
pub const REDIS_MYBAS_KUALA_TERENGGANU_MOTION_KEY: &str =
    "rapidbro:mybas-kuala-terengganu:buses:motion";
pub const REDIS_MYBAS_KUALA_TERENGGANU_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-kuala-terengganu:ingestor:last_ingest_at";

// ── GTFS-RT fetcher ───────────────────────────────────────────────────────────

pub async fn fetch_kuala_terengganu_bus_positions(
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
    let response = reqwest::get(MYBAS_KUALA_TERENGGANU_GTFS_RT_URL).await?;
    ...
    let positions: Vec<BusPosition> = feed
        ...
        .filter_map(|entity| {
            ...
            Some(BusPosition {
                ...
                accessibility: 0,
                busstop_id: None,
                provider: "MYBAS-KUALA-TERENGGANU".to_string(),
            })
        })
        .collect();
...
async fn get_kuala_terengganu_last_ingest(
    state: &AppState,
    ...
    let val: Option<i64> = redis::cmd("GET")
        .arg(REDIS_MYBAS_KUALA_TERENGGANU_INGEST_LAST_KEY)
...
pub async fn ensure_kuala_terengganu_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    // Quick check without lock
    let last_ingest_ms = get_kuala_terengganu_last_ingest(state).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < KUALA_TERENGGANU_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Acquire lock — only one fetch at a time
    let _guard = state.kuala_terengganu_fetch_lock.lock().await;

    // Re-check after acquiring lock (another request may have fetched while we waited)
    let last_ingest_ms = get_kuala_terengganu_last_ingest(state).await?;
    if let Some(last_ms) = last_ingest_ms {
        if now_unix_ms() - last_ms < KUALA_TERENGGANU_DATA_TTL_MS {
            return Ok(());
        }
    }
    ...
    let positions = fetch_kuala_terengganu_bus_positions()
        .await
        .map_err(|e| internal_error(format!("[mybas-kuala-terengganu] Fetch failed: {}", e)))?;

    println!(
        "[mybas-kuala-terengganu] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    .await
    .map_err(|e| {
        internal_error(format!("[mybas-kuala-terengganu] Redis write failed: {}", e))
    })?;

    Ok(())
}

// ── Kuala Terengganu Redis snapshot loader ────────────────────────────────────

async fn load_kuala_terengganu_bus_snapshot(
    state: &AppState,
...
pub async fn kuala_terengganu_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_kuala_terengganu_data_fresh(&state).await?;
    let snapshot = load_kuala_terengganu_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    ...
    println!(
        "[mybas-kuala-terengganu] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
...
pub async fn kuala_terengganu_get_route_eta(
    ...
    let route_stops =
        get_route_stops_from_cache(&route_id, state.kuala_terengganu_gtfs_cache.as_ref())
    ...
    println!(
        "[mybas-kuala-terengganu] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn kuala_terengganu_get_stop_eta(
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.kuala_terengganu_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-kuala-terengganu] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
...
pub async fn kuala_terengganu_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes =
        get_routes_for_stop_from_cache(&stop_id, state.kuala_terengganu_gtfs_cache.as_ref())
            .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-kuala-terengganu] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
...
pub async fn kuala_terengganu_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.kuala_terengganu_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!(
                "[mybas-kuala-terengganu] get_route_stops route={}",
                route_id
            );
            Ok(Json(response))
        }
...
pub async fn kuala_terengganu_get_route_shape(
    Path(route_id): Path<String>,
    Query(query): Query<RouteShapeQuery>,
    State(state): State<AppState>,
) -> Result<Json<RouteShapeResponse>, (StatusCode, Json<ErrorResponse>)> {
    let cache = state.kuala_terengganu_gtfs_cache.as_ref();
...
pub async fn kuala_terengganu_get_nearest_stop(
    ...
    println!(
        "[mybas-kuala-terengganu] get_nearest_stop lat={}, lon={} -> stop_id={}",
