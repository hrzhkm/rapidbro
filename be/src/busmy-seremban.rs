...
const MYBAS_SEREMBAN_A_GTFS_RT_URL: &str =
    "https://api.data.gov.my/gtfs-realtime/vehicle-position/mybas-seremban-a";
const MYBAS_SEREMBAN_B_GTFS_RT_URL: &str =
    "https://api.data.gov.my/gtfs-realtime/vehicle-position/mybas-seremban-b";

/// TTL for cached bus data: 5 minutes.
const SEREMBAN_DATA_TTL_MS: i64 = 300_000;

// ── Seremban A keys ───────────────────────────────────────────────────────────

pub const REDIS_MYBAS_SEREMBAN_A_LATEST_KEY: &str = "rapidbro:mybas-seremban-a:buses:latest";
pub const REDIS_MYBAS_SEREMBAN_A_LAST_SEEN_KEY: &str =
    "rapidbro:mybas-seremban-a:buses:last_seen";
pub const REDIS_MYBAS_SEREMBAN_A_MOTION_KEY: &str = "rapidbro:mybas-seremban-a:buses:motion";
pub const REDIS_MYBAS_SEREMBAN_A_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-seremban-a:ingestor:last_ingest_at";

// ── Seremban B keys ───────────────────────────────────────────────────────────

pub const REDIS_MYBAS_SEREMBAN_B_LATEST_KEY: &str = "rapidbro:mybas-seremban-b:buses:latest";
pub const REDIS_MYBAS_SEREMBAN_B_LAST_SEEN_KEY: &str =
    "rapidbro:mybas-seremban-b:buses:last_seen";
pub const REDIS_MYBAS_SEREMBAN_B_MOTION_KEY: &str = "rapidbro:mybas-seremban-b:buses:motion";
pub const REDIS_MYBAS_SEREMBAN_B_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-seremban-b:ingestor:last_ingest_at";

// ── GTFS-RT fetchers ──────────────────────────────────────────────────────────

pub async fn fetch_seremban_a_bus_positions(
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
    fetch_seremban_positions(MYBAS_SEREMBAN_A_GTFS_RT_URL, "MYBAS-SEREMBAN-A").await
}

pub async fn fetch_seremban_b_bus_positions(
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
    fetch_seremban_positions(MYBAS_SEREMBAN_B_GTFS_RT_URL, "MYBAS-SEREMBAN-B").await
}

async fn fetch_seremban_positions(
    url: &str,
    provider: &str,
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn ensure_seremban_a_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    let last_ingest_ms = get_last_ingest(state, REDIS_MYBAS_SEREMBAN_A_INGEST_LAST_KEY).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < SEREMBAN_DATA_TTL_MS {
            return Ok(());
        }
    }

    let _guard = state.seremban_a_fetch_lock.lock().await;

    let last_ingest_ms = get_last_ingest(state, REDIS_MYBAS_SEREMBAN_A_INGEST_LAST_KEY).await?;
    if let Some(last_ms) = last_ingest_ms {
        if now_unix_ms() - last_ms < SEREMBAN_DATA_TTL_MS {
            return Ok(());
        }
    }
    ...
    let positions = fetch_seremban_a_bus_positions()
        .await
        .map_err(|e| internal_error(format!("[mybas-seremban-a] Fetch failed: {}", e)))?;

    println!(
        "[mybas-seremban-a] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    if positions.is_empty() {
        let mut conn = state
            ...
            .map_err(internal_error)?;
        let _: () = redis::cmd("SET")
            .arg(REDIS_MYBAS_SEREMBAN_A_INGEST_LAST_KEY)
            .arg(now_unix_ms())
            .query_async(&mut conn)
            .await
            .map_err(internal_error)?;
    ...
    write_buses_to_redis_with_keys(
        ...
        &positions,
        now_unix_ms(),
        REDIS_MYBAS_SEREMBAN_A_LATEST_KEY,
        REDIS_MYBAS_SEREMBAN_A_LAST_SEEN_KEY,
        REDIS_MYBAS_SEREMBAN_A_MOTION_KEY,
        REDIS_MYBAS_SEREMBAN_A_INGEST_LAST_KEY,
    )
    .await
    .map_err(|e| internal_error(format!("[mybas-seremban-a] Redis write failed: {}", e)))?;

    Ok(())
}

pub async fn ensure_seremban_b_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    let last_ingest_ms = get_last_ingest(state, REDIS_MYBAS_SEREMBAN_B_INGEST_LAST_KEY).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < SEREMBAN_DATA_TTL_MS {
            return Ok(());
    ...
    let _guard = state.seremban_b_fetch_lock.lock().await;
    ...
    let positions = fetch_seremban_b_bus_positions()
        .await
        .map_err(|e| internal_error(format!("[mybas-seremban-b] Fetch failed: {}", e)))?;

    println!(
        "[mybas-seremban-b] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    if positions.is_empty() {
        let mut conn = state
            ...
            .map_err(internal_error)?;
        let _: () = redis::cmd("SET")
            .arg(REDIS_MYBAS_SEREMBAN_B_INGEST_LAST_KEY)
            .arg(now_unix_ms())
            .query_async(&mut conn)
            .await
            .map_err(internal_error)?;
    ...
    write_buses_to_redis_with_keys(
        ...
        &positions,
        now_unix_ms(),
        REDIS_MYBAS_SEREMBAN_B_LATEST_KEY,
        REDIS_MYBAS_SEREMBAN_B_LAST_SEEN_KEY,
        REDIS_MYBAS_SEREMBAN_B_MOTION_KEY,
        REDIS_MYBAS_SEREMBAN_B_INGEST_LAST_KEY,
    )
    .await
    .map_err(|e| internal_error(format!("[mybas-seremban-b] Redis write failed: {}", e)))?;

    Ok(())
}

// ── Redis snapshot loaders ────────────────────────────────────────────────────

async fn load_seremban_a_bus_snapshot(
    state: &AppState,
...
async fn load_seremban_b_bus_snapshot(
    state: &AppState,
) -> Result<RedisBusSnapshot, (StatusCode, Json<ErrorResponse>)> {
    load_active_bus_snapshot_with_keys(
        state,
...
// ── HTTP handlers — Seremban A ────────────────────────────────────────────────

pub async fn seremban_a_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_seremban_a_data_fresh(&state).await?;
    let snapshot = load_seremban_a_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    ...
    println!(
        "[mybas-seremban-a] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
...
pub async fn seremban_a_get_route_eta(
    ...
    let route_stops =
        get_route_stops_from_cache(&route_id, state.seremban_a_gtfs_cache.as_ref())
    ...
    println!(
        "[mybas-seremban-a] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn seremban_a_get_stop_eta(
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.seremban_a_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-seremban-a] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
...
pub async fn seremban_a_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes =
        get_routes_for_stop_from_cache(&stop_id, state.seremban_a_gtfs_cache.as_ref())
            .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-seremban-a] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
...
pub async fn seremban_a_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.seremban_a_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!("[mybas-seremban-a] get_route_stops route={}", route_id);
            Ok(Json(response))
        }
...
pub async fn seremban_a_get_nearest_stop(
    ...
    println!(
        "[mybas-seremban-a] get_nearest_stop lat={}, lon={} -> stop_id={}",
        query.lat, query.lon, response.stop_id
    );
    Ok(Json(response))
}

// ── HTTP handlers — Seremban B ────────────────────────────────────────────────

pub async fn seremban_b_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_seremban_b_data_fresh(&state).await?;
    let snapshot = load_seremban_b_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    let is_stale = match snapshot.last_ingest_at_unix_ms {
        Some(last_ingest_ms) => now_ms - last_ingest_ms > state.stale_after_ms,
        None => true,
    ...
    println!(
        "[mybas-seremban-b] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
    Ok(Json(GetAllResponse {
        data: snapshot.buses,
...
pub async fn seremban_b_get_route_eta(
    Path((route_id, stop_id)): Path<(String, String)>,
    State(state): State<AppState>,
) -> Result<Json<Vec<BusEta>>, (StatusCode, Json<ErrorResponse>)> {
    ensure_seremban_b_data_fresh(&state).await?;
    ...
    let visible_buses = filter_non_stationary_buses(&snapshot, state.stationary_window_ms);
    let route_stops =
        get_route_stops_from_cache(&route_id, state.seremban_b_gtfs_cache.as_ref())
            .map_err(|(status, msg)| (status, Json(ErrorResponse { error: msg })))?;

    let eta_results =
        calculate_route_eta_from_stops(&visible_buses, &route_id, &stop_id, &route_stops)?;
    println!(
        "[mybas-seremban-b] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn seremban_b_get_stop_eta(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<Vec<BusEta>>, (StatusCode, Json<ErrorResponse>)> {
    ensure_seremban_b_data_fresh(&state).await?;
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.seremban_b_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-seremban-b] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
    Ok(Json(all_eta_results))
...
pub async fn seremban_b_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes =
        get_routes_for_stop_from_cache(&stop_id, state.seremban_b_gtfs_cache.as_ref())
            .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-seremban-b] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
    Ok(Json(StopRoutesResponse { stop_id, routes }))
...
pub async fn seremban_b_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.seremban_b_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!("[mybas-seremban-b] get_route_stops route={}", route_id);
            Ok(Json(response))
        }
        Err((status, message)) => Err((status, Json(ErrorResponse { error: message }))),
    }
...
pub async fn seremban_b_get_route_shape(
    Path(route_id): Path<String>,
    Query(query): Query<RouteShapeQuery>,
    State(state): State<AppState>,
) -> Result<Json<RouteShapeResponse>, (StatusCode, Json<ErrorResponse>)> {
    let cache = state.seremban_b_gtfs_cache.as_ref();
    match get_shape_by_route(
        &route_id,
        query.stop_id.as_deref(),
        &cache.context.routes,
...
pub async fn seremban_b_get_nearest_stop(
    Query(query): Query<NearestStopQuery>,
    State(state): State<AppState>,
) -> Result<Json<NearestStopResponse>, (StatusCode, Json<ErrorResponse>)> {
    if !(-90.0..=90.0).contains(&query.lat) || !(-180.0..=180.0).contains(&query.lon) {
    ...
    println!(
        "[mybas-seremban-b] get_nearest_stop lat={}, lon={} -> stop_id={}",
        query.lat, query.lon, response.stop_id
    );
    Ok(Json(response))
}
