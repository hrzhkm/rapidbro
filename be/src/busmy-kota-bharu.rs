...
const MYBAS_KOTA_BHARU_GTFS_RT_URL: &str =
    "https://api.data.gov.my/gtfs-realtime/vehicle-position/mybas-kota-bharu";

/// TTL for cached bus data: 5 minutes.
const KOTA_BHARU_DATA_TTL_MS: i64 = 300_000;

pub const REDIS_MYBAS_KOTA_BHARU_LATEST_KEY: &str = "rapidbro:mybas-kota-bharu:buses:latest";
pub const REDIS_MYBAS_KOTA_BHARU_LAST_SEEN_KEY: &str =
    "rapidbro:mybas-kota-bharu:buses:last_seen";
pub const REDIS_MYBAS_KOTA_BHARU_MOTION_KEY: &str = "rapidbro:mybas-kota-bharu:buses:motion";
pub const REDIS_MYBAS_KOTA_BHARU_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-kota-bharu:ingestor:last_ingest_at";

// ── GTFS-RT fetcher ───────────────────────────────────────────────────────────

pub async fn fetch_kota_bharu_bus_positions(
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
    let response = reqwest::get(MYBAS_KOTA_BHARU_GTFS_RT_URL).await?;
    ...
    let positions: Vec<BusPosition> = feed
        ...
        .filter_map(|entity| {
            ...
            Some(BusPosition {
                ...
                accessibility: 0,
                busstop_id: None,
                provider: "MYBAS-KOTA-BHARU".to_string(),
            })
        })
        .collect();
...
async fn get_kota_bharu_last_ingest(
    state: &AppState,
    ...
    let val: Option<i64> = redis::cmd("GET")
        .arg(REDIS_MYBAS_KOTA_BHARU_INGEST_LAST_KEY)
...
pub async fn ensure_kota_bharu_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    // Quick check without lock
    let last_ingest_ms = get_kota_bharu_last_ingest(state).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < KOTA_BHARU_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Acquire lock — only one fetch at a time
    let _guard = state.kota_bharu_fetch_lock.lock().await;

    // Re-check after acquiring lock (another request may have fetched while we waited)
    let last_ingest_ms = get_kota_bharu_last_ingest(state).await?;
    if let Some(last_ms) = last_ingest_ms {
        if now_unix_ms() - last_ms < KOTA_BHARU_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Fetch from API
    let positions = fetch_kota_bharu_bus_positions().await.map_err(|e| {
        internal_error(format!("[mybas-kota-bharu] Fetch failed: {}", e))
    })?;

    println!(
        "[mybas-kota-bharu] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    .await
    .map_err(|e| internal_error(format!("[mybas-kota-bharu] Redis write failed: {}", e)))?;

    Ok(())
}

// ── Kota Bharu Redis snapshot loader ─────────────────────────────────────────

async fn load_kota_bharu_bus_snapshot(
    state: &AppState,
...
pub async fn kota_bharu_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_kota_bharu_data_fresh(&state).await?;
    let snapshot = load_kota_bharu_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    ...
    println!(
        "[mybas-kota-bharu] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
...
pub async fn kota_bharu_get_route_eta(
    ...
    let route_stops =
        get_route_stops_from_cache(&route_id, state.kota_bharu_gtfs_cache.as_ref())
    ...
    println!(
        "[mybas-kota-bharu] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn kota_bharu_get_stop_eta(
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.kota_bharu_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-kota-bharu] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
...
pub async fn kota_bharu_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes =
        get_routes_for_stop_from_cache(&stop_id, state.kota_bharu_gtfs_cache.as_ref())
            .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-kota-bharu] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
...
pub async fn kota_bharu_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.kota_bharu_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!("[mybas-kota-bharu] get_route_stops route={}", route_id);
            Ok(Json(response))
        }
...
pub async fn kota_bharu_get_nearest_stop(
    ...
    println!(
        "[mybas-kota-bharu] get_nearest_stop lat={}, lon={} -> stop_id={}",
