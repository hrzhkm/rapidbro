...
const KANGAR_DATA_TTL_MS: i64 = 300_000;
...
pub const REDIS_MYBAS_KANGAR_MOTION_KEY: &str = "rapidbro:mybas-kangar:buses:motion";
pub const REDIS_MYBAS_KANGAR_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-kangar:ingestor:last_ingest_at";

// ── GTFS-RT fetcher ───────────────────────────────────────────────────────────

pub async fn fetch_kangar_bus_positions(
    ...
    let positions: Vec<BusPosition> = feed
        ...
        .filter_map(|entity| {
            ...
            Some(BusPosition {
                ...
                accessibility: 0,
                busstop_id: None,
                provider: "MYBAS-KANGAR".to_string(),
            })
        })
        .collect();
...
async fn get_kangar_last_ingest(
    state: &AppState,
...
pub async fn ensure_kangar_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    // Quick check without lock
    let last_ingest_ms = get_kangar_last_ingest(state).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < KANGAR_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Acquire lock — only one fetch at a time
    let _guard = state.kangar_fetch_lock.lock().await;

    // Re-check after acquiring lock (another request may have fetched while we waited)
    let last_ingest_ms = get_kangar_last_ingest(state).await?;
    if let Some(last_ms) = last_ingest_ms {
        if now_unix_ms() - last_ms < KANGAR_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Fetch from API
    let positions = fetch_kangar_bus_positions().await.map_err(|e| {
        internal_error(format!("[mybas-kangar] Fetch failed: {}", e))
    })?;

    println!(
        "[mybas-kangar] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    .await
    .map_err(|e| internal_error(format!("[mybas-kangar] Redis write failed: {}", e)))?;

    Ok(())
}

// ── Kangar Redis snapshot loader ─────────────────────────────────────────────

async fn load_kangar_bus_snapshot(
    state: &AppState,
...
pub async fn kangar_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_kangar_data_fresh(&state).await?;
    let snapshot = load_kangar_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    ...
    println!(
        "[mybas-kangar] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
...
pub async fn kangar_get_route_eta(
    ...
    let route_stops = get_route_stops_from_cache(&route_id, state.kangar_gtfs_cache.as_ref())
    ...
    println!(
        "[mybas-kangar] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn kangar_get_stop_eta(
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.kangar_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-kangar] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
...
pub async fn kangar_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes = get_routes_for_stop_from_cache(&stop_id, state.kangar_gtfs_cache.as_ref())
        .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-kangar] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
...
pub async fn kangar_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.kangar_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!("[mybas-kangar] get_route_stops route={}", route_id);
            Ok(Json(response))
        }
...
pub async fn kangar_get_nearest_stop(
    ...
    println!(
        "[mybas-kangar] get_nearest_stop lat={}, lon={} -> stop_id={}",
