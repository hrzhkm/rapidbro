...
const MYBAS_MELAKA_GTFS_RT_URL: &str =
    "https://api.data.gov.my/gtfs-realtime/vehicle-position/mybas-melaka";

/// TTL for cached bus data: 5 minutes.
const MELAKA_DATA_TTL_MS: i64 = 300_000;

pub const REDIS_MYBAS_MELAKA_LATEST_KEY: &str = "rapidbro:mybas-melaka:buses:latest";
pub const REDIS_MYBAS_MELAKA_LAST_SEEN_KEY: &str = "rapidbro:mybas-melaka:buses:last_seen";
pub const REDIS_MYBAS_MELAKA_MOTION_KEY: &str = "rapidbro:mybas-melaka:buses:motion";
pub const REDIS_MYBAS_MELAKA_INGEST_LAST_KEY: &str =
    "rapidbro:mybas-melaka:ingestor:last_ingest_at";

// ── GTFS-RT fetcher ───────────────────────────────────────────────────────────

pub async fn fetch_melaka_bus_positions(
) -> Result<Vec<BusPosition>, Box<dyn std::error::Error + Send + Sync>> {
    let response = reqwest::get(MYBAS_MELAKA_GTFS_RT_URL).await?;
    ...
    let positions: Vec<BusPosition> = feed
        ...
        .filter_map(|entity| {
            ...
            Some(BusPosition {
                ...
                accessibility: 0,
                busstop_id: None,
                provider: "MYBAS-MELAKA".to_string(),
            })
        })
        .collect();
...
async fn get_melaka_last_ingest(
    state: &AppState,
    ...
    let val: Option<i64> = redis::cmd("GET")
        .arg(REDIS_MYBAS_MELAKA_INGEST_LAST_KEY)
...
pub async fn ensure_melaka_data_fresh(
    state: &AppState,
) -> Result<(), (StatusCode, Json<ErrorResponse>)> {
    // Quick check without lock
    let last_ingest_ms = get_melaka_last_ingest(state).await?;
    let now_ms = now_unix_ms();
    if let Some(last_ms) = last_ingest_ms {
        if now_ms - last_ms < MELAKA_DATA_TTL_MS {
            return Ok(());
        }
    }

    // Acquire lock — only one fetch at a time
    let _guard = state.melaka_fetch_lock.lock().await;

    // Re-check after acquiring lock (another request may have fetched while we waited)
    let last_ingest_ms = get_melaka_last_ingest(state).await?;
    if let Some(last_ms) = last_ingest_ms {
        if now_unix_ms() - last_ms < MELAKA_DATA_TTL_MS {
            return Ok(());
        }
    }
    ...
    let positions = fetch_melaka_bus_positions()
        .await
        .map_err(|e| internal_error(format!("[mybas-melaka] Fetch failed: {}", e)))?;

    println!(
        "[mybas-melaka] On-demand fetch: {} bus positions",
        positions.len()
    );
    ...
    write_buses_to_redis_with_keys(
        ...
        &positions,
        now_unix_ms(),
        REDIS_MYBAS_MELAKA_LATEST_KEY,
        REDIS_MYBAS_MELAKA_LAST_SEEN_KEY,
        REDIS_MYBAS_MELAKA_MOTION_KEY,
        REDIS_MYBAS_MELAKA_INGEST_LAST_KEY,
    )
    .await
    .map_err(|e| internal_error(format!("[mybas-melaka] Redis write failed: {}", e)))?;

    Ok(())
}

// ── Melaka Redis snapshot loader ──────────────────────────────────────────────

async fn load_melaka_bus_snapshot(
    state: &AppState,
...
pub async fn melaka_fetch_all_buses(
    State(state): State<AppState>,
) -> Result<Json<GetAllResponse>, (StatusCode, Json<ErrorResponse>)> {
    ensure_melaka_data_fresh(&state).await?;
    let snapshot = load_melaka_bus_snapshot(&state).await?;
    let now_ms = now_unix_ms();
    ...
    println!(
        "[mybas-melaka] fetch_all_buses: {} active buses",
        snapshot.buses.len()
    );
...
pub async fn melaka_get_route_eta(
    ...
    let route_stops = get_route_stops_from_cache(&route_id, state.melaka_gtfs_cache.as_ref())
    ...
    println!(
        "[mybas-melaka] get_route_eta route={}, stop={}: {} buses",
        route_id,
        stop_id,
        eta_results.len()
    );
...
pub async fn melaka_get_stop_eta(
    ...
    let all_eta_results = calculate_stop_eta_from_snapshot(
        &snapshot,
        state.melaka_gtfs_cache.as_ref(),
        &stop_id,
        state.stationary_window_ms,
    );

    println!(
        "[mybas-melaka] get_stop_eta stop={}: {} incoming buses",
        stop_id,
        all_eta_results.len()
    );
...
pub async fn melaka_get_stop_routes(
    Path(stop_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<StopRoutesResponse>, (StatusCode, Json<ErrorResponse>)> {
    let routes = get_routes_for_stop_from_cache(&stop_id, state.melaka_gtfs_cache.as_ref())
        .map_err(|(status, message)| (status, Json(ErrorResponse { error: message })))?;

    println!(
        "[mybas-melaka] get_stop_routes stop={}: {} routes",
        stop_id,
        routes.len()
    );
...
pub async fn melaka_get_route_stops(
    Path(route_id): Path<String>,
    State(state): State<AppState>,
) -> Result<Json<RouteStopsResponse>, (StatusCode, Json<ErrorResponse>)> {
    match get_route_stops_from_cache(&route_id, state.melaka_gtfs_cache.as_ref()) {
        Ok(response) => {
            println!("[mybas-melaka] get_route_stops route={}", route_id);
            Ok(Json(response))
        }
...
pub async fn melaka_get_nearest_stop(
    ...
    println!(
        "[mybas-melaka] get_nearest_stop lat={}, lon={} -> stop_id={}",
